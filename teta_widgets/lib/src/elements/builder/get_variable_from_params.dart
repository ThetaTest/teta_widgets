      ...
      if (paramsToSend?[page.defaultParams[i].id]['dataset'] == 'Text') {
        ...
        if (selectedDataset?['name'] == 'Parameters' ||
          ...
          try {
              ...
              if (element.keys.toList()[state.loop ?? 0] ==
                ...
                if (element[element.keys.toList()[state.loop ?? 0]] is XFile) {
                  variable = VariableObject(
                    ...
                    file: element[element.keys.toList()[state.loop ?? 0]]
                        as XFile,
                  );
                }
              }
            ...
            Logger.printError(
              'Error in getVariableObjectsFromParams func line 81, error: $e',
            );
          }
        } else {
          try {
            ...
            Logger.printError(
              'Error in getVariableObjectsFromParams func line 90, error: $e',
            );
            variables.add(
              page.defaultParams[i],
            );
          }
        }
        if (variable != null) {
