...
class MapControllerControl extends StatefulWidget {
  ...
  @override
  MapControllerControlState createState() => MapControllerControlState();
}

class MapControllerControlState extends State<MapControllerControl> {
  int? nodeId;
  bool? isUpdated;
  String? text;
            ...
            if (widget.value.type == FTextTypeEnum.param)
              CDropdown(
                ...
                items: widget.page.params
                    .where(
                      (element) => element.type == VariableType.mapController,
                    )
                    .map((e) => e.name)
                    .toList(),
                onChange: (newValue) {
